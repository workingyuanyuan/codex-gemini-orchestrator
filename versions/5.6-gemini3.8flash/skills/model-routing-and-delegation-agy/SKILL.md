...
| Gemini 3.8 Flash · high | Worker | 100 | 74.4 | 65.4 | 57.4 | 43.4 | 83.1 |

† Cost Efficiency reflects subscription-adjusted cost.

The Gemini 3.8 Flash row temporarily carries forward the Gemini 3.7 Flash High routing estimates, including Cost Efficiency; these are provisional references, not measured 3.8 scores. Medium remains the default worker. Tier-specific 3.8 capability differences are unverified, so preserve the configured Medium/High routes without inferring new scores or measured capability equality.

## Default routes

- **Gemini 3.8 Flash Medium:** default for bounded, objectively verifiable execution. Keep coding and agentic work on Medium under the retained routing policy.
- **Gemini 3.8 Flash High:** upgrade from Medium only when the acceptance bar specifically requires the High reasoning tier. Do not upgrade for Coding or Agentic requirements alone.
...
```powershell
& "$HOME\.codex\scripts\Invoke-AntigravityAgent.ps1" -WorkingDirectory $worktree -Model gemini-3.8-flash -PromptFile $contract -AutoApprove
```
