...
param(
    ...
    [Parameter(Mandatory)]
    [ValidateSet('gemini-3.5-flash', 'gemini-3.1-pro')]
    [string]$Model,
...
$modelPattern = switch ($Model) {
    'gemini-3.5-flash' { 'Gemini\s+3\.5\s+Flash\s+\(High\)' }
    'gemini-3.1-pro'   { 'Gemini\s+3\.1\s+Pro\s+\(High\)' }
    default { throw "Unsupported model alias: $Model" }
}

$resolvedModels = @(
    [regex]::Matches($cleanModelsOutput, $modelPattern, [Text.RegularExpressions.RegexOptions]::IgnoreCase) |
        ForEach-Object { $_.Value } |
        Sort-Object -Unique
)

if ($resolvedModels.Count -ne 1) {
    throw "Model alias '$Model' did not resolve uniquely. Expected one High model match; found $($resolvedModels.Count). Available model output:`n$cleanModelsOutput"
}
